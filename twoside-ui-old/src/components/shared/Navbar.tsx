import { Home, ArrowLeftRight, Plus, LogOut, HandCoins } from "lucide-react";
...
export default function Navbar({ active_tab, set_active_tab, on_open_action_sheet }: NavbarProps) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 bg-surface/85 backdrop-blur-xl border-t border-white/5 px-4 py-3 flex items-center justify-between max-w-md mx-auto">
      {/* Home Tab */}
      <button
        onClick={() => {
          navigate("/");
          if (set_active_tab) set_active_tab("home");
        }}
      ...
      <button
        onClick={() => {
          navigate("/loans");
          if (set_active_tab) set_active_tab("loans");
        }}
        className={`flex flex-col items-center gap-1 transition-colors ${
          active_tab === "loans" ? "text-primary" : "text-muted hover:text-zinc-200"
        }`}
      >
      ...
      <button
        onClick={() => {
          navigate("/");
          if (set_active_tab) set_active_tab("history");
        }}
        className={`flex flex-col items-center gap-1 transition-colors ${
