...
export function PickerSheet(props: PickerSheetProps) {
  return (
    <Sheet open={props.open} on_close={props.on_close} title={props.title}>
      {/* State lives in PickerSheetBody, which Sheet mounts only while open, so
          search / create-draft reset naturally on each open — no effect needed. */}
      <PickerSheetBody {...props} />
    </Sheet>
  );
}

function PickerSheetBody(props: PickerSheetProps) {
  const {
    items,
    selected_id,
    on_select,
