...
import { TRANSACTION_TYPE_META } from "@/constants/transactionTypeMeta";
...
export function TransactionRow({ entry, currency_symbol }: TransactionRowProps) {
  const meta = TRANSACTION_TYPE_META[entry.log_type];
  const Icon = meta.icon;
  const is_inflow = entry.side === "DEBIT"; // asset account: debit = money in

  // const title =
  //   entry.log_type === "TRANSFER" && entry.related_account
  //     ? `Transfer to ${entry.related_account.name}`
  //     : entry.related_counterparty
  //       ? `${meta.label} ${entry.log_type === "GIVE_LOAN" || entry.log_type === "REPAY_LOAN" ? "to" : "from"} ${entry.related_counterparty.name}`
  //       : entry.description;

  return (
	  <div className="py-3 flex items-center justify-between gap-3">
	    <div className="flex items-center gap-2 min-w-0">
	      <div className={`w-10 h-10 rounded-full ${meta.text} flex items-center justify-center shrink-0`}>
