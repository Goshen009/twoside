...
import { Plus, Trash2, Wallet, Search, Calendar, FileText, ChevronRight, ArrowUpRight } from "lucide-react";
...
interface RepayOutFormProps {
  accounts: AccountBalance[];
  onClose: () => void;
}

// Mock active borrowings (loans you took)
const MOCK_ACTIVE_BORROWINGS = [
  { id: "b1", counterparty: "Bank / Microfinance", description: "Short-term liquidity", amount: 150000, date: "2026-08-01" },
  { id: "b2", counterparty: "Sarah Williams", description: "Quick support", amount: 40000, date: "2026-08-12" },
];

export default function RepayOutForm({ accounts, onClose }: RepayOutFormProps) {
  ...
  // Bottom Sheet Picker State ("borrowing" | "account" | null)
  const [activePicker, setActivePicker] = useState<"borrowing" | "account" | null>(null);
  ...
  const selectedBorrowing = MOCK_ACTIVE_BORROWINGS.find((b) => b.id === selectedBorrowingId);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 relative">
      {/* Borrowing Selector Field */}
      <div className="space-y-1.5">
        <label className="text-[10px] font-mono text-muted uppercase tracking-wider">Select Loan to Repay / Settle</label>
        <button
          type="button"
          onClick={() => {
            setActivePicker("borrowing");
            setSearchQuery("");
          }}
        ...
        >
          <div className="flex items-center gap-2 truncate">
            ...
            <span className="text-zinc-100 truncate">
              {selectedBorrowing ? `${selectedBorrowing.counterparty} — ₦${selectedBorrowing.amount.toLocaleString()} (${selectedBorrowing.description})` : "Select active borrowing..."}
            </span>
          </div>
      ...
      <div className="space-y-1.5">
        ...
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="e.g., Final balance clearance..."
            value={description}
      ...
      <div className="space-y-2.5 pt-1">
        ...
        <div className="flex items-center justify-between px-3 py-2.5 rounded-xl bg-surface/60 border border-white/5 font-mono text-xs">
          <span className="text-muted text-[10px] uppercase tracking-wider">Total Repayment Out</span>
      ...
      >
        {isSubmitting ? "Saving..." : "Save Repay Out"}
      </button>
        ...
        <div className="absolute inset-0 z-50 bg-surface/95 backdrop-blur-md rounded-3xl p-4 flex flex-col space-y-3 border border-white/10 animate-in fade-in zoom-in-95 duration-150">
          <div className="flex items-center justify-between gap-2 pb-1">
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder={activePicker === "account" ? "Search accounts..." : "Search active borrowings..."}
                value={searchQuery}
          ...
          <div className="flex-1 overflow-y-auto space-y-1">
            ...
            {activePicker === "borrowing" &&
              MOCK_ACTIVE_BORROWINGS.filter(
                (b) =>
                  b.counterparty.toLowerCase().includes(searchQuery.toLowerCase()) ||
                ...
                >
                  <div className="space-y-0.5 pr-2">
                    <div className="text-xs font-medium text-zinc-200">{b.counterparty}</div>
                    <div className="text-[10px] text-muted truncate">{b.description} • {b.date}</div>
                  </div>
